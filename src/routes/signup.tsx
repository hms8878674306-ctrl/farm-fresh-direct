...
export const Route = createFileRoute("/signup")({
  head: () => ({ meta: [{ title: "Sign up — KrishiDirect" }] }),
  component: Signup,
});
  ...
  return (
    <main className="mx-auto max-w-md px-4 py-16">
      <div className="text-center mb-8">
        ...
        <h1 className="display text-3xl font-extrabold">Join KrishiDirect</h1>
        <p className="text-sm text-muted-foreground">Fresh from farm, straight to you</p>
      </div>
      ...
      <form onSubmit={submit} className="rounded-2xl bg-card border border-border p-6 space-y-3">
        ...
        <input required type="email" placeholder="Email" value={email} onChange={e=>setEmail(e.target.value)}
          className="w-full h-11 px-4 rounded-xl bg-background border border-border focus:border-primary focus:outline-none" />
        <input required type="password" placeholder="Password (min 6 chars)" value={pw} onChange={e=>setPw(e.target.value)}
          className="w-full h-11 px-4 rounded-xl bg-background border border-border focus:border-primary focus:outline-none" />
